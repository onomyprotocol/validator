...
Validators should not be allowed to join set with a sufficient stake. Testnet manager to supply NOM to those
stakeholders wishing to run the validator. This testnet is for stakeholders only with 225k+ NOM. Anyone may run a node,
...
* Get tokens from master account (***!!! Only in case your account wasn't set to the genesis file. If it was set - skip
  that step !!!***).
    * Request 250k noms (225k min stake for validator, 1k for orchestrator and 1k for transactions payments) tokens for
    ...
    ```
    onomyd tx bank send {sender-address} {validator-address} 250000000000000000000000anom --gas auto --gas-adjustment 1.5 --chain-id=onomy-testnet-1 --keyring-backend pass
    ```
...
* Send 1k noms tokens from you validator to your orchestrator
    ...
    ```
    onomyd tx bank send $(onomyd keys show validator -a --keyring-backend pass) $(onomyd keys show eth-orchestrator -a --keyring-backend pass) 1000000000000000000000anom --gas auto --gas-adjustment 1.5 --chain-id=onomy-testnet-1 --keyring-backend pass
    ```
...
```
onomyd tx slashing unjail --from <validator-name> --chain-id=onomy-testnet-1 --gas auto --gas-adjustment 1.5 --keyring-backend pass
```
